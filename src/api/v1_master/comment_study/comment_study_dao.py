...
from src.api.v1_master.study_board_comment.study_board_comment_dto import UpdateComment, ReadComment, CreateComment, ReadCommentlist
...
# Update
async def update_study_board_comment(study_board_no: int, study_board_comment_no: int, study_board_comment_info: Optional[CreateComment], db: AsyncSession):
    await db.execute(update(Study_Board_Comment).filter(Study_Board_Comment.Board_no == study_board_no).filter(Study_Board_Comment.Comment_no == study_board_comment_no).values(study_board_comment_info.dict()))
    await db.commit()
...
async def upload_update_study_board_comment(study_board_no: int, study_board_comment_no: int, content: str, file_name: list[UploadFile], db: AsyncSession) -> None:
    create_values = {
    "Board_no": study_board_no,
    "Comment_no" : study_board_comment_no,
    "Content": content,
    "Create_date": datetime.now(timezone.utc).replace(second=0, microsecond=0).replace(tzinfo=None),
    "Likes": 0  # 예시로 Likes 컬럼이 있다면
    }
...
async def delete_study_board_comment(study_board_no: int, study_board_comment_no: int, db: AsyncSession) -> None:
    await delete_image_study_board_comment(study_board_no, study_board_comment_no, db)
    await db.execute(delete(Study_Board_Comment).filter(Study_Board_Comment.Board_no == study_board_no).filter(Study_Board_Comment.Comment_no == study_board_comment_no))
    await db.commit()
...
# Delte Image
async def delete_image_study_board_comment(study_board_no: int, study_board_comment_no: int, db: AsyncSession) -> None:
    result = await db.execute(select(Study_Board_Comment.Image_paths).filter(Study_Board_Comment.Board_no == study_board_no).filter(Study_Board_Comment.Comment_no == study_board_comment_no))
