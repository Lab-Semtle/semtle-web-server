...
from src.api.v1_master.study_board_comment import study_board_comment_svc
...
async def create_study_board_comment(
    ...
    await study_board_comment_svc.create_study_board_comment(study_board_no, study_board_comment_info, db)
    return SU.CREATED
...
async def update_study_board_comment(
    ...
    study_board_comment_info: Optional[UpdateComment],
    db: AsyncSession = Depends(get_db)
):
    logger.info("----------스터디 게시판 기존 게시물 댓글 수정----------")
    await study_board_comment_svc.update_study_board_comment(study_board_comment_no, study_board_comment_info, db)
    return SU.SUCCESS
...
async def delete_study_board_comment(
    ...
    await study_board_comment_svc.delete_study_board_comment(study_board_no, study_board_comment_no, db)
    return SU.SUCCESS
