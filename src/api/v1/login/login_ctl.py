...
async def post_login(response: Response, login_form: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_db)):
    logger.info("----------로그인----------")
    # 사용자 인증 확인
    verify = await login_svc.verify(login_form.username, login_form.password, db)
    if not verify:
...
async def post_signup(login_info: Optional[CreateUserInfo]):
    logger.info("----------회원가입----------")
    
    # 사용자 존재 여부 확인
    if login_info and await login_svc.is_user(login_info.user_id, login_info.user_name, login_info.user_email, login_info.user_phone):
