...
async def find_pw(
    ...
    # 비밀번호가 발견되면 반환
    return ResultType(status='success', message=SU.SUCCESS[1], detail={"password": password})
...
async def find_email(
    ...
    # 이메일이 발견되면 반환
    return ResultType(status='success', message=SU.SUCCESS[1], detail={"email": email})
