...
#     try:
#         post = db.query(Board).filter(
#             and_(Board.no == post_no, Board.del_yn == "Y")).first()
...
#     try:
#         post = db.query(Board).filter(
#             and_(Board.tags == tags, Board.del_yn == "Y")).first()
#         return Post(no=post.no, like=post.like, dislike=post.dislike, writer=post.writer, title=post.title, tags=post.tags, content=post.content, date=post.date)
#     except Exception as e:
#         raise HTTPException(status_code=404, detail=f'에러({e})')
...
#     post = db.query(Board).filter(
#         and_(Board.no == update_post.no, Board.del_yn == "Y")).first()
#     try:
#         if not post:
...
#     post = db.query(Board).filter(
#         and_(Board.no == post_no, Board.del_yn == "N")).first()
#     try:
#         if not post:
...
#         cursor.execute(
#             "INSERT INTO files (filename, filepath) VALUES (?, ?)", (filename, file_path))
#         conn.commit()
...
#     try:
#         cursor.execute(
#             "SELECT filename, filepath FROM files WHERE id=?", (file_id,))
...
#     post = db.query(Board).filter(
#         and_(Board.no == comment.no, Board.del_yn == "Y")).first()
#     try:
#         if not post:
#             raise Exception("존재하지 않는 게시글 번호입니다.")
