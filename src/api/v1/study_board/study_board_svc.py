...
async def get_study_board_list(skip: int) -> list[ReadBoardlist]:
    total, study_board_info = await study_board_dao.get_study_board_list(skip)
...
async def update_study_board(study_board_no: int, study_board_info: Optional[UpdateBoard], select: bool) -> None:
    if not select: # defalt: 기존 이미지 삭제
        await study_board_dao.delete_image_study_board(study_board_no) # 기존에 저장된 이미지 삭제
    await study_board_dao.update_study_board(study_board_no, study_board_info)
...
async def upload_update_file_study_board(study_board_no: int, file_name: list[UploadFile], select: bool) -> None:
    if not select: # defalt: 기존 이미지 삭제
        ...
        await study_board_dao.upload_file_add_study_board(study_board_no, file_name)
    
# Delete
async def delete_study_board(study_board_no: int) -> None:
    await study_board_dao.delete_image_study_board(study_board_no)
    await study_board_comment_dao.all_delete_study_board_comment(study_board_no)
    await study_board_dao.delete_study_board(study_board_no)

# sort_Title
async def sort_study_board(skip: int, select: int) -> list[ReadBoardlist]:
    total, study_board_info = await study_board_dao.sort_study_board(skip, select)
    study_board_info = [ReadBoard.from_orm(board).dict() for board in study_board_info] 
    return total, study_board_info