...
@router.get(
    ...
    response_model=list[ReadComment],
    responses=Status.docs(SU.SUCCESS, ER.NOT_FOUND)
)
# 함수명 get, post, update, delete 중 1택 + 목적에 맞게 이름 작성
async def get_Free_Board_Comment(Free_Board_no: int, db: AsyncSession = Depends(get_db)):
    # 개발 중 logging 사용하고 싶을 때 이 코드 추가
    logger.info("----------자유 게시판 게시물 댓글 조회----------")
    Free_Board_Comment_info = await Free_Board_Comment_svc.get_Free_Board_Comment(Free_Board_no, db)
    return Free_Board_Comment_info
...
async def create_Free_Board_Comment(
    ...
    await Free_Board_Comment_svc.create_Free_Board_Comment(Free_Board_Comment, db)
    return SU.CREATED
